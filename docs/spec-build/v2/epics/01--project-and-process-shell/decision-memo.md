...
| Artifact/source attachment context | Split the shell contract by asset type: artifacts get an explicit `processContext` that can separately surface `currentProcessReference` and `producingProcess`, while source attachments keep project-vs-process attachment scope plus process label when process-scoped | Lock now | Without this, the shell becomes ambiguous once a project has multiple processes and teaches the wrong artifact ownership model. The shell still does not need full drill-down, but it does need enough context to answer "why is this here?" safely. |
