...
| Current Epic 3 artifact write path overwrites `artifacts.contentStorageId` rather than preserving prior revisions; AC-2.1 ("earlier version remains available for review") cannot be satisfied against that schema | `convex/artifacts.ts` (Epic 3 closure chunks); AC-2.1, AC-2.3 | **Shape A — direct schema change to Epic 3's artifacts table.** Trim `artifacts` to project-level identity only (`projectId`, `displayName`, new `createdAt`). Remove `processId`, `contentStorageId`, `currentVersionLabel`, and `updatedAt`. Version provenance lives on `artifactVersions.createdByProcessId`, and process reviewability is derived from process refs / pinned review context rather than artifact-row ownership. Single source of truth for "current artifact content" = `latest artifactVersions row by createdAt`. Epic 4 owns this storage model change because pre-customer stance in Epic 3's addendum permits direct schema edits (no user-data preservation required) and the removed fields have no long-term callers yet. | Resolved — deviated; later reinforced by Epic 5 alignment |
...
| Default package member selection was "first member by position," but AC-4.3c needed to distinguish directly renderable members from durable-but-unsupported ones | AC-4.3c | Default selection resolves to `members.find(m => m.status === 'ready') ?? members.find(m => m.status === 'unsupported')`. Falls through to unavailable only in pathological read paths, which are treated as unavailable rather than as a normal selection state. | Resolved — clarified |
| Package exportability signal is consumed by the client (to hide the export trigger for non-exportable packages) but was not defined on the `PackageReviewTarget` contract | AC-5.1b | Add `exportability: { available: boolean; reason?: string }` to `PackageReviewTarget`. Computed server-side from member durability, not from in-workspace renderability alone: unsupported-but-durable members remain exportable; unavailable members or zero-durable-member snapshots do not. Client renders the trigger only when `available === true`. | Resolved — clarified |
...
Epic 5 later settled several artifact-model questions that early Epic 4 drafts
still described with pre-alignment language. The normative reading for this
Epic 4 tech-design pack is:

- artifacts are project-scoped durable identities; no artifact row has one
  primary process owner
- process provenance lives at the artifact-version level
  (`artifactVersions.createdByProcessId`)
- reviewability is computed from the current process review context
  (process-referenced artifacts and explicitly pinned targets/packages), not
  from artifact-row ownership
- a package opened from one process review context may pin artifact versions
  produced by multiple processes in the same project
- zero-version artifacts remain excluded from `availableTargets` and do not
  enable review entry; they render as `status: empty` only when reached through
  a valid direct target path inside the current process review context
- review-workspace bootstrap prefers a bounded `200` unavailable target state
  once project/process context resolves; request-level `404
  REVIEW_TARGET_NOT_FOUND` remains for target-specific endpoints that cannot
  return a workspace envelope and for expired/tampered export-download URLs
...
The second posture change is durability. MDV treats documents as mutable local files. Liminal Build treats every artifact as a durable, versioned, project-level record with version-level process provenance. Epic 4 introduces two new durable concerns — `artifactVersions` as the versioned content pointer (one row per revision, each row pointing at its own Convex File Storage blob), and `packageSnapshots` plus `packageSnapshotMembers` as the durable, ordered, stable-forever record of what was published as one reviewable set. Epic 4 also modifies Epic 3's artifact schema: `contentStorageId`, `currentVersionLabel`, `updatedAt`, and the old artifact-level process attachment all come off the `artifacts` row, relocating to or being derived from `artifactVersions` or process-reference state. What remains on `artifacts` is project-scoped identity only (`projectId`, `displayName`, and a new `createdAt`). Epic 3's checkpoint path is updated to insert a new version row rather than overwrite in place. A package published today must still point at the same artifact versions after a member is revised tomorrow; that "pinned at publication" property is fundamental to review semantics and is expressed directly in the schema. Reviewability for a package requires at least one durable member at read time, and `publishPackageSnapshot` rejects zero-member publication input so the happy path never creates a memberless snapshot. Package membership is not constrained to artifacts last produced by the opening process; one package may pin versions from multiple processes in the same project. Epic 4 does not own *when* packages get published or *what goes in them* — that decision lives in downstream process-module epics. What Epic 4 owns is the storage contract, the typed internal mutations that downstream callers use to write to the tables, and the test coverage that proves the contract is correct.
...
After the migration, `artifacts` carries only project-scoped identity fields: `projectId`, `displayName`, and a new `createdAt` field. Previously-stored `processId`, `contentStorageId`, `currentVersionLabel`, and `updatedAt` are removed. No version-specific state remains on the row, and reviewability is no longer inferred from an artifact-row process attachment.
...
`packageSnapshots` stores the process review context that published the
snapshot plus `displayName`, `packageType`, and `publishedAt`.
`packageSnapshotMembers` stores `packageSnapshotId`, `position`, `artifactId`,
and **`artifactVersionId`** — the durable pin. A package published today must
still point at the same member versions after a later revision lands, and the
version id on each member row is what makes that property inviolate. Pinned
members may come from multiple processes in the same project. Both tables are
immutable after `publishPackageSnapshot` writes them transactionally.

Epic 3's checkpoint writer path is rewritten as part of this work: instead of overwriting `artifacts.contentStorageId`, it calls `insertArtifactVersion(artifactId, contentStorageId, versionLabel, contentKind, bytes, createdByProcessId)` — a typed internal mutation Epic 4 introduces. Epic 4 does not own the *decision* to publish a package (that lives in downstream process-module epics), but it does own the typed `publishPackageSnapshot(processId, displayName, packageType, members)` internal mutation that process-module code calls when it decides to publish from a given process review context.
...
A package is a **durable snapshot published from one process review context over a fixed ordered set of artifact versions**. Membership is captured by `packageSnapshotMembers` rows, each of which pins (`packageSnapshotId`, `position`, `artifactId`, `artifactVersionId`). The `(artifactId, artifactVersionId)` pair is the source of truth for what the package contains — revising the artifact later publishes a new `artifactVersions` row, but the snapshot's member row continues pointing at the original version id. Members may pin versions produced by multiple processes in the same project. Snapshots do not mutate.
...
| Export package action | `POST /api/projects/:projectId/processes/:processId/review/packages/:packageId/export` | Phase 1 of export: returns signed download metadata for a `.mpkz` archive; the subsequent GET to `/review/exports/:exportId` performs the actual byte stream |

The existing process work-surface bootstrap (`GET /api/projects/:projectId/processes/:processId`) is *not* changed structurally by Epic 4. The `review` action's enablement is driven inside `apps/platform/server/services/processes/process-work-surface.service.ts` — specifically the existing `case 'review':` branch that computes `{ enabled, disabledReason }` from process status. Epic 4 extends that branch to also consult reviewability (does this process review context currently reference at least one artifact with a durable version, or at least one package snapshot with at least one durable member?) before setting `enabled: true`. The shell-level process-list projection in `apps/platform/server/services/projects/platform-store.ts` gets the same reviewability check applied to its `availableActions` list so the shell and the process surface agree on whether `review` is offered. The `ProcessSummary` contract adds no new field; enablement stays private to the server's controls-computation logic and evolves there if the rule changes later.
...
| `convex/artifacts.ts` | **MODIFIED (storage model change)** | Table shape trims to project-scoped artifact identity: `projectId`, `displayName`, and a new `createdAt` field. **Removed fields: `processId`, `contentStorageId`, `currentVersionLabel`, `updatedAt`** — version-specific state relocates to or is derived from `artifactVersions`, and process linkage moves to process-reference / version-provenance surfaces. Epic 3's checkpoint writer path is rewritten to insert a new `artifactVersions` row via `insertArtifactVersion` rather than overwrite in place. Every caller of the removed fields is rewritten to use `getLatestArtifactVersion`. See `tech-design-server.md` §Artifact Storage Model Change for the exact field-by-field delta. | AC-2 |
...
├── artifacts.ts                                   # MODIFIED (project-scoped identity: projectId/displayName/createdAt; processId/contentStorageId/currentVersionLabel/updatedAt removed)
...
| Chunk 4 | Package review workspace: typed `publishPackageSnapshot` internal mutation (tested via Convex test infrastructure, no Epic 4 production caller); `packageSnapshots` + `packageSnapshotMembers` reads; package review service with first-ready-member preference plus durable-member fallback; `PackageMemberReview` envelope with status/error; member list; package context preservation; member-failure degradation; zero-member snapshots rejected on publish and treated as unavailable on read | AC-4.1 through AC-4.4 | `publishPackageSnapshot`, packageSnapshots reads, package review service, package review panel |
| Chunk 5 | Package export (two-phase): streaming `createPackageFromEntries` in the workspace package; tar hardening (gzip-bomb cap, per-entry + cumulative byte caps, entry-name filter); Fastify POST export (returns signed-URL JSON) + GET download (streams bytes); signed URL mint + verify; expired-URL 404; unsupported-but-durable members remain exportable while unavailable members block export; `mdvpkg` CLI polish | AC-5.1 through AC-5.3 | export service, export-url-signing, `@liminal-build/markdown-package/tar/create-from-entries.ts`, export trigger UI |
...
- **Artifact storage model change**: `artifacts` trims to project-scoped identity (`projectId`, `displayName`, new `createdAt`). Removed fields: `processId`, `contentStorageId`, `currentVersionLabel`, `updatedAt` — version-specific state relocates to `artifactVersions`, and process linkage moves to process-reference / version-provenance surfaces. Epic 3's checkpoint writer path is rewritten to insert a new version row per revision rather than overwrite the artifact row. This is a schema rewrite under the repo's pre-customer stance; no user-data-preservation migration dance is performed.
- **Package publication context**: Epic 4 ships the typed `publishPackageSnapshot` internal mutation and the immutability invariants that go with it (transactional snapshot + members insert; no update API). The publishing process anchors review entry and reopen semantics, but package members may pin artifacts from multiple processes in the same project. Epic 4 does not ship a production caller — downstream process-module epics decide when and what to publish, and call this API when their logic fires.
...
- **Exportability tracks durability, not workspace renderability alone**: a package is exportable when it has at least one durable member and no member is unavailable. Members with `status: unsupported` stay exportable because the archive packages pinned bytes, not in-workspace render output.
...
- **Review enablement stays server-side**: the `review` action's `controls[i].enabled` is computed by the existing controls projection, extended to consult reviewability in the current process review context (does the process currently reference at least one artifact with a durable version, or at least one package snapshot with at least one durable member?). No new field on the `ProcessSummary` contract; the shared process-summary shape stays narrow.
- **Bootstrap error taxonomy**: review-workspace bootstrap returns a bounded `200` unavailable target state when project/process context resolves but the selected target no longer resolves in that context. Request-level `404 REVIEW_TARGET_NOT_FOUND` remains for target-specific endpoints and expired/tampered export-download URLs.
- **Default package member selection**: when a package has no explicit `memberId` in the query state, selection defaults to the first member with `status === 'ready'`; if no member is ready, it falls through to the first durable member (`status === 'unsupported'`) so the package can still open. A snapshot with no durable members is not reviewable and is treated as unavailable rather than as an empty package state.
