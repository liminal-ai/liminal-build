...
| AC-2.4 | TC-2.4a | Story 2 | `tests/service/server/artifact-review-api.test.ts`; `tests/service/client/artifact-review-panel.test.ts` | Direct artifact URL only, through a valid process review context; zero-version artifacts remain excluded from target lists by Story 1 |
...
| AC-6.2 | TC-6.2a, TC-6.2b, TC-6.2c | Story 6 | `tests/service/server/review-workspace-api.test.ts`; `tests/service/client/review-workspace-page.test.ts` | Uses request-level blocking for revoked access and bounded `REVIEW_TARGET_NOT_FOUND` target errors for unavailable workspace targets after process/project context still resolves |
...
| Path 1.4 | Reach a zero-version artifact only by direct artifact URL inside a valid process review context and show the no-version state | Story 2 | TC-2.4a | `tests/service/server/artifact-review-api.test.ts`; `tests/service/client/artifact-review-panel.test.ts` |
