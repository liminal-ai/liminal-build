...
The environment-state matrix test cases `TC-1.1c` through `TC-1.1k` remain in Story 1 as first-load visibility and disabled-control expectations for seeded or durable environment states. Story 5 owns the product flows and recovery mutations that make recovery-only states operationally reachable.
...
| Path 2.2 | Hydrate the current working set from current artifacts, outputs, and sources | Story 2 | TC-2.2a, TC-2.2b |
...
| Path 4.1 | Persist artifact outputs durably to canonical process state | Story 4 | TC-4.1a |
...
| Path 5.2 | Rehydrate a recoverable environment from canonical materials | Story 5 | TC-5.2a, TC-5.2b |
| Path 5.3 | Rebuild a lost or unusable environment from canonical truth | Story 5 | TC-5.3a, TC-5.3b |
| Path 5.4 | Preserve durable artifact and code truth through recovery | Story 5 | TC-5.4a, TC-5.4b |
...
| Path 6.2 | Restore prior checkpointed artifact and code results without an active environment after reopen | Story 6 | TC-6.2a |
