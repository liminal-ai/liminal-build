...
describe('process work surface api', () => {
  ...
  it('TC-4.3a and TC-4.4b bootstrap reads only the current material refs, so stale prior rows do not return and shared materials can surface', async () => {
    ...
    const sharedCurrentSource = buildSourceAttachmentSummaryFixture({
      ...
      accessMode: 'read_only' as const,
      repositoryUrl: 'https://github.com/liminal-ai/research-repo',
      targetRef: 'main',
      ...
      currentSources: [
        {
          ...
          repositoryUrl: sharedCurrentSource.repositoryUrl,
          repositoryFullName: sharedCurrentSource.repositoryFullName,
    ...
    expect(response.json().materials.currentSources).toEqual([
      {
        ...
        repositoryUrl: writableSource.repositoryUrl,
        repositoryFullName: writableSource.repositoryFullName,
        attachmentScope: writableSource.attachmentScope,
  ...
  it('S2-TC-2.5b: bootstrap exposes read_only accessMode for an attached read-only source', async () => {
    const readOnlySource = buildSourceAttachmentSummaryFixture({
      ...
      purpose: 'research' as const,
      accessMode: 'read_only' as const,
      repositoryUrl: 'https://github.com/liminal-ai/research-notes',
      targetRef: 'main',
      hydrationState: 'hydrated' as const,
      attachmentScope: 'project' as const,
      processId: null,
    ...
    expect(response.json().materials.currentSources).toEqual([
      {
        ...
        accessMode: 'read_only',
        repositoryUrl: readOnlySource.repositoryUrl,
        repositoryFullName: readOnlySource.repositoryFullName,
